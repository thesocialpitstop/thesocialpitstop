...
const PartnersComponent = () => {
  ...
  const {
    ...
    error: partnersError,
  } = useQuery(LIST_PARTNERS_OF_USER, {
    variables: {
    ...
    if (data.partner_status == "pending") {
      return (
        <ActionContainer>
          ...
          <Button
            variant="contained"
            onClick={() => acceptPartner(data.partner_id)}
            sx={{ bgcolor: "green", height: "3rem" }}
          >
            Accept
          </Button>
          <Button
            variant="contained"
            onClick={() => rejectPartner(data.partner_id)}
            sx={{ bgcolor: "red", height: "3rem" }}
          >
            Reject
          </Button>
        </ActionContainer>
  ...
  if (partners.queryUserWithItemTypePrefix.items.length == 0)
